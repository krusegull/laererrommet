...
import { importantNoteUpdateSchema } from "@/lib/validations";

async function assertOwnership(id: string, userId: string) {
  const note = await prisma.importantNote.findUnique({ where: { id } });
...
export async function PATCH(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const parsed = importantNoteUpdateSchema.safeParse(body);
  if (!parsed.success) {
  ...
  const note = await prisma.importantNote.update({
    where: { id },
    data: parsed.data,
  });
...
export async function DELETE(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  await prisma.importantNote.delete({ where: { id } });
